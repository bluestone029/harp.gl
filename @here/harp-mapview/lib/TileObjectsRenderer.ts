    ...
    setupRenderer() {
        ...
        const stableSort = (a: RenderItem, b: RenderItem): number => {
            if (a.groupOrder !== b.groupOrder) {
                ...
                a.object.userData.tileKey !== undefined &&
                b.object.userData.tileKey !== undefined &&
                a.object.userData.tileKey?.mortonCode() !== b.object.userData.tileKey?.mortonCode()
            ) {
                return (
                    a.object.userData.tileKey?.mortonCode() -
                    b.object.userData.tileKey?.mortonCode()
                );
        ...
        const painterSortStable = (a: RenderItem, b: RenderItem): number => {
            ...
            const techniqueA = mapObjectAdapterA?.technique;
            const techniqueB = mapObjectAdapterB?.technique;
            const isRoadA = (techniqueA as IndexedTechnique)?._category?.startsWith("road");
            const isRoadB = (techniqueB as IndexedTechnique)?._category?.startsWith("road");
            ...
            if (mapObjectAdapterA?.level && mapObjectAdapterB?.level) {
                const isRoad = isRoadA ?? isRoadB;
                if (isRoad) {
